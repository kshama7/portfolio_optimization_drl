...
def BaselineStats(
    ...
    baseline_start="2024-02-28",
    baseline_end="2024-04-10",
):

    nsei, dow_strat = baseline_strat(
...
def baseline_strat(ticker, start, end):
    nsei = YahooDownloader(
        start_date=start, end_date=end, ticker_list=[ticker]
    ).fetch_data()
    nsei["daily_return"] = nsei["close"].pct_change(1)
    dow_strat = backtest_strat(nsei)
    return nsei, dow_strat
